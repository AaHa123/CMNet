...
def train(train_loader, model, optimizer, epoch, save_path, writer):
    ...
    try:
        for i, (images, gts) in enumerate(train_loader, start=1):
            ...
            # ---- forward ----
            lateral_map_3, lateral_map_2, lateral_map_1, lateral_map_c = model(images)
            # ---- loss function ----
            loss3 = structure_loss(lateral_map_3, gts)
            loss2 = structure_loss(lateral_map_2, gts)
            loss1 = structure_loss(lateral_map_1, gts)
            lossc = structure_loss(lateral_map_c, gts)
            ...
            if i % 20 == 0 or i == total_step or i == 1:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], '
                      '[lateral-3: {:.4f}], [lateral-2: {:.4f}], [lateral-1: {:.4f}],[lateral-c: {:.4f}]'.
        ...
        if epoch % 80 == 0:
            torch.save(model.state_dict(), save_path + 'Net_{}.pth'.format(epoch))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path + 'Net_{}.pth'.format(epoch + 1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_root', type=str, default='./data/TrainDataset/', help='the training rgb images root')
    parser.add_argument('--val_root', type=str, default='./data/TestDataset/', help='the test rgb images root')
    parser.add_argument('--save_path', type=str, default='./checkpoints/Net_cod/', help='the path to save model and log')
    # parser.add_argument('--net_channel', type=int, default=64)
    ...
    train_loader = get_loader(image_root=opt.train_root + 'Imgs/',
                              ...
                              num_workers=8)
    '''
    val_loader_camo = test_dataset(image_root=opt.val_root + 'COD10K/Image/',
                                   gt_root=opt.val_root + 'COD10K/GT/',
                                   testsize=opt.trainsize)
    '''
    val_loader_camo = test_dataset(image_root=opt.val_root + 'CAMO/Image/',
    ...
    for epoch in range(1, opt.epoch):
        ...
        val(val_loader_camo, model, epoch, save_path, writer)